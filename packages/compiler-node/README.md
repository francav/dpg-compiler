# @francav/compiler-node

Node.js entrypoint for the DPG compiler. It re-exports the host-agnostic
analysis from `@francav/compiler-core` and adds file-loading helpers so you can
...
```sh
npm install @francav/compiler-node
```

## Usage

```ts
import { compileFromFiles } from "@francav/compiler-node";
...
`@francav/compiler-core` for callers that already hold the model XML in memory.
